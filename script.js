...
document.getElementById("generate-report").addEventListener("click", () => {
    ...
    const addSectionTitle = (title) => {
        if (yOffset > 270) addPage(); // Ensure page breaks before starting a section
    ...
    const addText = (text) => {
        const splitText = doc.splitTextToSize(text, 180); // Wrap text within the page width
    ...
    doc.line(margin, yOffset, 200, yOffset); // Add a line below the title
    yOffset += 10;

    // Project Details Section
    addSectionTitle("Project Details");
    addText(`Project Name: ${document.getElementById("project-name").value || "N/A"}`);
    addText(`Location: ${document.getElementById("location").value || "N/A"}`);
    addText(`Assessor Name: ${document.getElementById("assessor-name").value || "N/A"}`);
    addText(`Date of Assessment: ${document.getElementById("date").value || "N/A"}`);
    addText(`Project Description: ${document.getElementById("project-description").value || "N/A"}`);
    addText(`Project Purpose: ${document.getElementById("project-purpose").value || "N/A"}`);

    // Site Information Section
    addSectionTitle("Site Information");
    addText(`Flora & Fauna: ${document.getElementById("flora-fauna").value || "N/A"}`);
    const environmentalFeatures = Array.from(document.getElementById("environmental-features").selectedOptions)
        .map(option => option.value).join(", ");
    addText(`Environmental Features: ${environmentalFeatures || "N/A"}`);
    addText(`Distance to Sensitive Ecosystems (km): ${document.getElementById("distance-ecosystem").value || "N/A"}`);
    addText(`Distance to Critical Infrastructure (km): ${document.getElementById("distance-infrastructure").value || "N/A"}`);
    addText(`Land Use History: ${document.getElementById("land-use-history").value || "N/A"}`);
    addText(`Current Site Activities: ${document.getElementById("current-activities").value || "N/A"}`);
    addText(`Site Accessibility: ${document.getElementById("accessibility").value || "N/A"}`);
    ...
    addText(recommendationsText);

    // Radar Chart Section
    addPage(); // Ensure the chart is on a new page
